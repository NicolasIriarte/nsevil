...
(defmacro nsevil-without-display (&rest body)
  "Execute BODY without Nsevil displays.
Inhibits echo area messages, mode line updates and cursor changes."
  (declare (indent defun)
           (debug t))
  `(let ((nsevil-no-display t))
     ,@body))

(defvar nsevil-cached-header-line-height nil
  "Cached height of the header line.
Used for fallback implementation on older Emacsen.")

(defun nsevil-header-line-height ()
...
(defun nsevil-posn-x-y (position)
    ...
    (when header-line-format
      (setcdr xy (+ (cdr xy)
                    (or (and (fboundp 'window-header-line-height)
                        ...
                        nsevil-cached-header-line-height
                        (setq nsevil-cached-header-line-height (nsevil-header-line-height))))))
    xy))

(defun nsevil-count-lines (beg end)
...
(defun nsevil-move-beginning-of-line (&optional arg)
...
(defun nsevil-move-end-of-line (&optional arg)
...
(defun nsevil-adjust-cursor (&optional _)
  "Move point one character back if at the end of a non-empty line.
This behavior is controled by `nsevil-move-beyond-eol'."
  (when (and (eolp)
             ...
             (= (point)
                (save-excursion
                  ...
                  (point))))
    (nsevil-move-cursor-back t)))

(defun nsevil-move-cursor-back (&optional force)
  "Move point one character back within the current line.
Contingent on the variable `nsevil-move-cursor-back' or the FORCE
...
to the current field as recognized by `line-beginning-position'."
  (when (or nsevil-move-cursor-back force)
...
(defun nsevil-line-position (line &optional column)
...
A negative number means the end of the line."
  (save-excursion
    (when (fboundp 'nsevil-goto-line)
      (nsevil-goto-line line))
    (if (numberp column)
        (if (< column 0)
            (beginning-of-line 2)
...
(defun nsevil-column (&optional pos)
...
(defun nsevil-move-to-column (column &optional dir force)
...
(defmacro nsevil-with-hproject-point-on-window (&rest body)
...
(defun nsevil-forward-not-thing (thing &optional count)
  "Move point to the end or beginning of the complement of THING."
  (nsevil-motion-loop (dir (or count 1))
    (let (bnd)
      (cond
       ((> dir 0)
...
(defun nsevil-bounds-of-not-thing-at-point (thing &optional which)
...
(defun nsevil-forward-nearest (count &rest forwards)
...
(defun bounds-of-nsevil-string-at-point (&optional state)
      ...
      (and (nth 3 state)
           (cons (nth 8 state)
                 (and (parse-partial-sexp (point)
                                          ...
                                          'syntax-table)
                      (point)))))))
(put 'nsevil-string 'bounds-of-thing-at-point #'bounds-of-nsevil-string-at-point)

(defun bounds-of-nsevil-comment-at-point ()
  "Return the bounds of a string at point."
  (save-excursion
      ...
      (and (nth 4 state)
           (cons (nth 8 state)
                 (and (parse-partial-sexp (point)
                                          ...
                                          'syntax-table)
                      (point)))))))
(put 'nsevil-comment 'bounds-of-thing-at-point #'bounds-of-nsevil-comment-at-point)
...
(defun nsevil-forward-syntax (syntax &optional count)
...
(defun nsevil-forward-chars (chars &optional count)
...
(defun nsevil-up-block (beg end &optional count)
...
(defun nsevil-up-paren (open close &optional count)
...
last successful match (that caused COUNT to reach zero)."
  ...
  (let (forward-sexp-function)
    (with-syntax-table (copy-syntax-table (syntax-table))
      ...
      (let ((rest (nsevil-motion-loop (dir count)
                    (let ((pnt (point)))
                      (condition-case nil
                          (cond
                           ((> dir 0)
...
(defun nsevil-up-xml-tag (&optional count)
...
(defun nsevil-forward-quote (quote &optional count)
  ...
  (let (reset-parser)
    (with-syntax-table (copy-syntax-table (syntax-table))
      ...
      (let* ((pnt (point))
             ...
             (bnd (bounds-of-nsevil-string-at-point state)))
         ...
         ((> count 0)
          ...
          (catch 'done
            (while (and (> count 0) (not (eobp)))
              ...
              (cond
               ((nth 3 state)
                (setq bnd (bounds-of-thing-at-point 'nsevil-string))
                (goto-char (cdr bnd))
                (setq count (1- count)))
...
;;; Thing-at-point motion functions for Nsevil text objects and motions
(defun forward-nsevil-empty-line (&optional count)
...
(defun forward-nsevil-space (&optional count)
  "Move forward COUNT whitespace sequences [[:space:]]+."
  (nsevil-forward-chars "[:space:]" count))

(defun forward-nsevil-word (&optional count)
...
an empty line matching ^$."
  (nsevil-forward-nearest
   count
   #'(lambda (&optional cnt)
       (let ((word-separating-categories nsevil-cjk-word-separating-categories)
             (word-combining-categories nsevil-cjk-word-combining-categories)
             (pnt (point)))
         (forward-word cnt)
         (if (= pnt (point)) cnt 0)))
   #'(lambda (&optional cnt)
       (nsevil-forward-chars "^[:word:]\n\r\t\f " cnt))
   #'forward-nsevil-empty-line))
...
'[^\\n\\r\\t\\f ]', or an empty line matching ^$."
  (nsevil-forward-nearest count
                        #'(lambda (&optional cnt)
                            (nsevil-forward-chars "^\n\r\t\f " cnt))
                        #'forward-nsevil-empty-line))

(defun forward-nsevil-symbol (&optional count)
...
(defun forward-nsevil-defun (&optional count)
...
(defun forward-nsevil-sentence (&optional count)
...
(defun forward-nsevil-paragraph (&optional count)
...
(defvar nsevil-forward-quote-char ?\"
  "The character to be used by `forward-nsevil-quote'.")

(defun forward-nsevil-quote (&optional count)
  "Move forward COUNT strings.
The quotation character is specified by the global variable
`nsevil-forward-quote-char'. This character is passed to
`nsevil-forward-quote'."
  (nsevil-forward-quote nsevil-forward-quote-char count))

(defun forward-nsevil-quote-simple (&optional count)
  "Move forward COUNT strings.
The quotation character is specified by the global variable
`nsevil-forward-quote-char'. This functions uses Vim's rules
...
;;; Motion functions
(defun nsevil-forward-beginning (thing &optional count)
...
(defun nsevil-backward-beginning (thing &optional count)
  "Move backward to beginning of THING.
The motion is repeated COUNT times. This is the same as calling
`nsevil-backward-beginning' with -COUNT."
  (nsevil-forward-beginning thing (- (or count 1))))

(defun nsevil-forward-end (thing &optional count)
...
(defun nsevil-backward-end (thing &optional count)
  "Move backward to end of THING.
The motion is repeated COUNT times. This is the same as calling
`nsevil-backward-end' with -COUNT."
  (nsevil-forward-end thing (- (or count 1))))

(defun nsevil-forward-word (&optional count)
  "Move by words.
Moves point COUNT words forward or (- COUNT) words backward if
...
(defun nsevil-in-comment-p (&optional pos)
...
(defun nsevil-looking-at-start-comment (&optional move)
...
of the comment opener if MOVE is non-nil."
  (cond
   ...
   ((= (char-syntax (char-after)) ?<)
    (equal (point) (nsevil-in-comment-p (1+ (point)))))
   ;; two character opener on first char
   ((and (not (zerop (logand (car (syntax-after (point)))
                             (lsh 1 16))))
         (not (zerop (logand (or (car (syntax-after (1+ (point)))) 0)
                             (lsh 1 17)))))
    (equal (point) (nsevil-in-comment-p (+ 2 (point)))))
   ;; two character opener on second char
   ((and (not (zerop (logand (car (syntax-after (point)))
                             (lsh 1 17))))
         (not (zerop (logand (or (car (syntax-after (1- (point)))) 0)
                             (lsh 1 16)))))
    (and (equal (1- (point)) (nsevil-in-comment-p (1+ (point))))
         (prog1 t (when move (backward-char)))))))

(defun nsevil-looking-at-end-comment (&optional move)
...
closer if MOVE is non-nil."
  (cond
   ;; one char closer
   ((= (char-syntax (char-after)) ?>)
    (and (nsevil-in-comment-p) ; in comment
         (not (nsevil-in-comment-p (1+ (point))))
   ...
   ((and (not (zerop (logand (car (syntax-after (point)))
         ...
         (not (zerop (logand (or (car (syntax-after (1+ (point)))) 0)
                             (lsh 1 19)))))
    (and (nsevil-in-comment-p)
         (not (nsevil-in-comment-p (+ (point) 2)))
...
(defun nsevil-yank-line-handler (text)
  "Inserts the current text linewise."
  (let ((text (apply #'concat (make-list (or nsevil-paste-count 1) text)))
        (opoint (point)))
    (nsevil-remove-yank-excluded-properties text)
    (cond
     ((eq this-command 'nsevil-paste-before)
      (nsevil-move-beginning-of-line)
      (nsevil-move-mark (point))
      (insert text)
      (setq nsevil-last-paste
            (list 'nsevil-paste-before
                  ...
                  (mark t)
                  (point)))
      (nsevil-set-marker ?\[ (mark))
      (nsevil-set-marker ?\] (1- (point)))
      (nsevil-exchange-point-and-mark)
      (back-to-indentation))
     ((eq this-command 'nsevil-paste-after)
      (nsevil-move-end-of-line)
      (nsevil-move-mark (point))
      (insert "\n")
      (insert text)
      (nsevil-set-marker ?\[ (1+ (mark)))
      (nsevil-set-marker ?\] (1- (point)))
      ...
      (setq nsevil-last-paste
            (list 'nsevil-paste-after
                  ...
                  (mark t)
                  (point)))
      (nsevil-move-mark (1+ (mark t)))
      (nsevil-exchange-point-and-mark)
      (back-to-indentation))
     (t
      (insert text)))))

(defun nsevil-yank-block-handler (lines)
  "Inserts the current text as block."
  (let ((count (or nsevil-paste-count 1))
        (col (if (eq this-command 'nsevil-paste-after)
                 (1+ (current-column))
               (current-column)))
    ...
    (dolist (line lines)
      ...
      (let ((text (match-string 1 line))
        ...
        (unless (and (< (nsevil-column (line-end-position)) col)
                     (zerop (length text)))
          ;; if we paste behind eol, it may be sufficient to insert tabs
          (if (< (nsevil-column (line-end-position)) col)
    ...
    (nsevil-set-marker ?\] (1- epoint))
    (goto-char opoint)
    (when (and (eq this-command 'nsevil-paste-after)
               (not (eolp)))
      (forward-char))))

(defun nsevil-delete-yanked-rectangle (nrows ncols)
  "Special function to delete the block yanked by a previous paste command."
  (let ((opoint (point))
        (col (if (eq last-command 'nsevil-paste-after)
                 (1+ (current-column))
               (current-column))))
    (dotimes (_ nrows)
...
;; buffers where `nsevil-paste-pop' may be useful have undoing enabled.
...
`nsevil-paste-before', `nsevil-paste-after' or `nsevil-paste-pop'.
This command uses the same paste command as before, i.e., when
used after `nsevil-paste-after' the new text is also yanked using
`nsevil-paste-after', used with the same paste-count argument.
...
is negative this is a more recent kill."
  ...
  (unless (memq last-command
                '(nsevil-paste-after
                  nsevil-paste-before
                  nsevil-visual-paste))
    (user-error "Previous command was not an nsevil-paste: %s" last-command))
  (unless nsevil-last-paste
    (user-error "Previous paste command used a register"))
  (nsevil-undo-pop)
  (goto-char (nth 2 nsevil-last-paste))
  (setq this-command (nth 0 nsevil-last-paste))
  ;; use temporary kill-ring, so the paste cannot modify it
  (let ((kill-ring (list (current-kill
                          (if (and (> count 0) (nth 5 nsevil-last-paste))
                              ;; if was visual paste then skip the
                              ;; text that has been replaced
                              (1+ count)
                            count))))
        (kill-ring-yank-pointer kill-ring))
    (when (eq last-command 'nsevil-visual-paste)
      (let ((nsevil-no-display t))
        (nsevil-visual-restore)))
    (funcall (nth 0 nsevil-last-paste) (nth 1 nsevil-last-paste))
    ;; if this was a visual paste, then mark the last paste as NOT
    ;; being the first visual paste
    (when (eq last-command 'nsevil-visual-paste)
      (setcdr (nthcdr 4 nsevil-last-paste) nil))))

(defun nsevil-paste-pop-next (count)
  "Same as `nsevil-paste-pop' but with negative argument."
  (interactive "p")
  (nsevil-paste-pop (- count)))
