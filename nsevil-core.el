;;; nsevil-core.el --- Core functionality -*- lexical-binding: t -*-
...
;; Nsevil is defined as a globalized minor mode, enabled with the toggle
;; function `nsevil-mode'.  This in turn enables `nsevil-local-mode' in
...
;;
;; Within this hierarchy, Nsevil arranges the keymaps for the current
...
;;
;; These keymaps are listed in `nsevil-mode-map-alist', which is listed
;; in `emulation-mode-map-alist'.
;;
;; Most of the key bindings for a state are stored in its global
;; keymap, which has a name such as `nsevil-normal-state-map'. (See the
;; file nsevil-maps.el, which contains all the default key bindings.) A
;; state also has a local keymap (`nsevil-normal-state-local-map'),
...
;; own by passing the mode's keymap to the function `nsevil-define-key'
;; or `nsevil-define-minor-mode-key'. The former uses a specific map to
...
;; to emulation status by passing it to `nsevil-make-intercept-map' or
;; `nsevil-make-overriding-map'.  An "intercept" keymap has priority over
;; all other Nsevil keymaps.  (Nsevil uses this facility when debugging and
...
;; shadowed by Nsevil's default bindings.)
;;
;; States are defined with the macro `nsevil-define-state', which
;; creates a command for switching to the state.  This command,
;; for example `nsevil-normal-state' for Normal state, performs
;; the following tasks:
;;
;;     * Setting `nsevil-state' to the new state.
;;     * Refreshing the keymaps in `nsevil-mode-map-alist'.
;;     * Updating the mode line.
;;       - Normal state depends on `nsevil-normal-state-tag'.
;;     * Adjusting the cursor's appearance.
;;       - Normal state depends on `nsevil-normal-state-cursor'.
;;     * Displaying a message in the echo area.
;;       - Normal state depends on `nsevil-normal-state-message'.
;;     * Running hooks.
;;       - Normal state runs `nsevil-normal-state-entry-hook' when
;;         entering, and `nsevil-normal-state-exit-hook' when exiting.
;;
...
;; to the `nsevil-state-property' function.  Nsevil defines the states
...
(declare-function nsevil-emacs-state-p "nsevil-states")
(declare-function nsevil-ex-p "nsevil-ex")
(defvar nsevil-mode-buffers)

(define-minor-mode nsevil-local-mode
  "Minor mode for setting up Nsevil in a single buffer."
  :init-value nil
  (cond
   ((nsevil-disabled-buffer-p)
    ;; Don't leave the mode variable on in buffers where nsevil disabled, because
    ;; functions that check this variable will get an incorrect result (e.g.,
    ;; nsevil-refresh-cursor).
    (setq nsevil-local-mode nil))
   (nsevil-local-mode
    (setq emulation-mode-map-alists
          (nsevil-concat-lists '(nsevil-mode-map-alist)
                             emulation-mode-map-alists))
    (nsevil-initialize-local-keymaps)
    ;; restore the proper value of `major-mode' in Fundamental buffers
    (when (eq major-mode 'turn-on-nsevil-mode)
      (setq major-mode 'fundamental-mode))
    (when (minibufferp)
      (setq-local nsevil-default-state 'insert)
      (setq-local nsevil-echo-state nil))
    ;; The initial state is usually setup by `nsevil-initialize' when
    ;; the major-mode in a buffer changes. This preliminary
    ;; initialization is only for the case when `nsevil-local-mode' is
    ;; called directly for the first time in a buffer.
    (unless nsevil-state (nsevil-initialize-state))
    (add-hook 'input-method-activate-hook 'nsevil-activate-input-method t t)
    (add-hook 'input-method-deactivate-hook 'nsevil-deactivate-input-method t t)
    ;; (add-hook 'activate-mark-hook 'nsevil-visual-activate-hook nil t)
    ;; (add-hook 'pre-command-hook 'nsevil-repeat-pre-hook)
    ;; (add-hook 'post-command-hook 'nsevil-repeat-post-hook)
    )
   (t
    (nsevil-refresh-mode-line)
    ;; (remove-hook 'activate-mark-hook 'nsevil-visual-activate-hook t)
    (remove-hook 'input-method-activate-hook 'nsevil-activate-input-method t)
    (remove-hook 'input-method-deactivate-hook 'nsevil-deactivate-input-method t)
    (nsevil-change-state nil))))
...
;; variables.
(put 'nsevil-local-mode 'permanent-local t)

(defun turn-on-nsevil-mode (&optional arg)
  "Turn on Nsevil in the current buffer."
  (interactive)
  (nsevil-local-mode (or arg 1)))

(defun turn-off-nsevil-mode (&optional arg)
  "Turn off Nsevil in the current buffer."
  (interactive)
  (nsevil-local-mode (or arg -1)))

;; The function `nsevil-initialize' should only be used to initialize
;; `nsevil-local-mode' from the globalized minor-mode `nsevil-mode'. It is
;; called whenever nsevil is enabled in a buffer for the first time or
;; when nsevil is active and the major-mode of the buffer changes. In
;; addition to enabling `nsevil-local-mode' it also sets the initial
;; nsevil-state according to the major-mode.
(defun nsevil-initialize ()
  "Enable Nsevil in the current buffer, if appropriate.
To enable Nsevil globally, do (nsevil-mode 1)."
  (unless (and (minibufferp) (not nsevil-want-minibuffer))
    (nsevil-local-mode 1)
    (nsevil-initialize-state)))

;;;###autoload (autoload 'nsevil-mode "nsevil" nil t)
(define-globalized-minor-mode nsevil-mode
  nsevil-local-mode nsevil-initialize)

;; No hooks are run in Fundamental buffers, so other measures are
;; necessary to initialize Nsevil in these buffers. When Nsevil is
;; enabled globally, the default value of `major-mode' is set to
;; `turn-on-nsevil-mode', so that Nsevil is enabled in Fundamental
;; buffers as well. Then, the buffer-local value of `major-mode' is
;; changed back to `fundamental-mode'. (Since the `nsevil-mode' function
;; is created by a macro, we use `defadvice' to augment it.)
(defadvice nsevil-mode (after start-nsevil activate)
  "Enable Nsevil in Fundamental mode."
  (if nsevil-mode
      (progn
        (when (eq (default-value 'major-mode) 'fundamental-mode)
          ;; changed back by `nsevil-local-mode'
          (setq-default major-mode 'turn-on-nsevil-mode))
        (ad-enable-regexp "^nsevil")
        (ad-activate-regexp "^nsevil")
        (with-no-warnings (nsevil-esc-mode 1)))
    (when (eq (default-value 'major-mode) 'turn-on-nsevil-mode)
      (setq-default major-mode 'fundamental-mode))
    (ad-disable-regexp "^nsevil")
    (ad-update-regexp "^nsevil")
    (with-no-warnings (nsevil-esc-mode -1))))

(defun nsevil-change-state (state &optional message)
  "Change the state to STATE.
If STATE is nil, disable all states."
  (let ((func (nsevil-state-property (or state nsevil-state) :toggle)))
    (when (and (functionp func)
               (or message (not (eq state nsevil-state))))
      (funcall func (if state (and message 1) -1)))))

(defmacro nsevil-save-state (&rest body)
  "Save the current state; execute BODY; restore the state."
  (declare (indent defun)
           (debug t))
  `(let* ((nsevil-state nsevil-state)
          (nsevil-previous-state nsevil-previous-state)
          (nsevil-previous-state-alist (copy-tree nsevil-previous-state-alist))
          (nsevil-next-state nsevil-next-state)
          (old-state nsevil-state)
          (inhibit-quit t)
          (buf (current-buffer)))
     (unwind-protect
         (progn ,@body)
       (when (buffer-live-p buf)
         (with-current-buffer buf
           (nsevil-change-state old-state))))))

(defmacro nsevil-with-state (state &rest body)
  "Change to STATE and execute BODY without refreshing the display.
Restore the previous state afterwards."
  (declare (indent defun)
           (debug t))
  `(nsevil-without-display
     (nsevil-save-state
       (nsevil-change-state ',state)
       ,@body)))

(defun nsevil-initializing-p (&optional buffer)
  "Whether Nsevil is in the process of being initialized."
  (memq (or buffer (current-buffer)) nsevil-mode-buffers))

(defun nsevil-initialize-state (&optional state buffer)
  "Set up the initial state for BUFFER.
BUFFER defaults to the current buffer.
Uses STATE if specified, or calls `nsevil-initial-state-for-buffer'.
See also `nsevil-set-initial-state'."
  (with-current-buffer (or buffer (current-buffer))
    (if state (nsevil-change-state state)
      (nsevil-change-to-initial-state buffer))))
(put 'nsevil-initialize-state 'permanent-local-hook t)

(defun nsevil-initial-state-for-buffer-name (&optional name default)
  "Return the initial Nsevil state to use for a buffer with name NAME.
Matches the name against the regular expressions in
`nsevil-buffer-regexps'. If none matches, returns DEFAULT."
  (let ((name (if (stringp name) name (buffer-name name)))
        regexp state)
    (when (stringp name)
      (catch 'done
        (dolist (entry nsevil-buffer-regexps default)
          (setq regexp (car entry)
                state (cdr entry))
          (when (string-match regexp name)
            (throw 'done state)))))))

(defun nsevil-disabled-buffer-p (&optional buffer)
  "Whether Nsevil should be disabled in BUFFER."
  (null (nsevil-initial-state-for-buffer-name buffer 'undefined)))

(defun nsevil-initial-state-for-buffer (&optional buffer default)
  "Return the initial Nsevil state to use for BUFFER.
BUFFER defaults to the current buffer. Returns DEFAULT
if no initial state is associated with BUFFER.
See also `nsevil-initial-state'."
  (with-current-buffer (or buffer (current-buffer))
    (or (nsevil-initial-state-for-buffer-name (buffer-name))
        (catch 'done
          (dolist (mode minor-mode-map-alist)
            (setq mode (car-safe mode))
            (when (and (boundp mode) (symbol-value mode))
              (when (setq mode (nsevil-initial-state mode))
                (throw 'done mode)))))
        (nsevil-initial-state major-mode nil t)
        default)))

(defun nsevil-initial-state (mode &optional default follow-parent checked-modes)
  "Return the Nsevil state to use for MODE or its alias.
Returns DEFAULT if no initial state is associated with MODE.
The initial state for a mode can be set with
`nsevil-set-initial-state'.
  ...
  (cond
   ...
   ((and mode (symbolp mode))
    (let ((mode-alias (let ((func (symbol-function mode)))
      ...
      (or
       (catch 'done
         (dolist (entry (nsevil-state-property t :modes) default)
           ...
           (when (or (memq mode modes)
             ...
             (throw 'done state))))
       (when follow-parent
         (nsevil-initial-state (get mode 'derived-mode-parent)
                             nil t (cons mode checked-modes)))
       (when follow-parent
         (nsevil-initial-state (get mode-alias 'derived-mode-parent)
                             nil t (cons mode-alias checked-modes))))))))

(defun nsevil-set-initial-state (mode state)
  "Set the initial state for major mode MODE to STATE.
This is the state the buffer comes up in."
  (dolist (modes (nsevil-state-property t :modes))
    (setq modes (cdr-safe modes))
    (set modes (delq mode (symbol-value modes))))
  (when state
    (add-to-list (nsevil-state-property state :modes) mode)))

(nsevil-define-command nsevil-change-to-initial-state
  (&optional buffer message)
  "Change the state of BUFFER to its initial state.
This is the state the buffer came up in. If Nsevil is not activated
then this function does nothing."
  :keep-visual t
  :suppress-operator t
  (with-current-buffer (or buffer (current-buffer))
    (when nsevil-local-mode
      (nsevil-change-state (nsevil-initial-state-for-buffer
                          buffer (or nsevil-default-state 'normal))
                         message))))

(nsevil-define-command nsevil-change-to-previous-state
  (&optional buffer message)
  ...
  :suppress-operator t
  (with-current-buffer (or buffer (current-buffer))
    (let ((prev-state nsevil-previous-state)
          (prev-prev-state (cdr-safe (assoc nsevil-previous-state
                                            nsevil-previous-state-alist))))
      (nsevil-change-state nil)
      (when prev-prev-state
        (setq nsevil-previous-state prev-prev-state))
      (nsevil-change-state (or prev-state nsevil-default-state 'normal)
                         message))))

;; When a buffer is created in a low-level way, it is invisible to
;; Nsevil (as well as other globalized minor modes) because no hooks are
...
;; otherwise, though, so advise this function to initialize Nsevil.
(defadvice set-window-buffer (before nsevil)
  "Initialize Nsevil in the displayed buffer."
  (when nsevil-mode
    (when (get-buffer (ad-get-arg 1))
      (with-current-buffer (ad-get-arg 1)
        (unless nsevil-local-mode
          (nsevil-local-mode 1))))))

;; Refresh cursor color.
;; Cursor color can only be set for each frame but not for each buffer.
(add-hook 'window-configuration-change-hook 'nsevil-refresh-cursor)
(defadvice select-window (after nsevil activate)
  (nsevil-refresh-cursor))

(defun nsevil-generate-mode-line-tag (&optional state)
  "Generate the nsevil mode-line tag for STATE."
  (let ((tag (nsevil-state-property state :tag t)))
    (when (functionp tag)
      (setq tag (funcall tag)))
    ;; prepare mode-line: add tooltip
    (if (stringp tag)
        (propertize tag
                    'help-echo (nsevil-state-property state :name)
                    'mouse-face 'mode-line-highlight)
      tag)))

(defun nsevil-refresh-mode-line (&optional state)
  "Refresh mode line tag."
  (when (listp mode-line-format)
    (setq nsevil-mode-line-tag (nsevil-generate-mode-line-tag state))
    ;; refresh mode line data structure
    ;; first remove nsevil from mode-line
    (setq mode-line-format (delq 'nsevil-mode-line-tag mode-line-format))
    (let ((mlpos mode-line-format)
          pred which where)
      ;; determine before/after which symbol the tag should be placed
      (cond
       ((eq nsevil-mode-line-format 'before)
        (setq where 'after which 'mode-line-position))
       ((eq nsevil-mode-line-format 'after)
        (setq where 'after which 'mode-line-modes))
       ((consp nsevil-mode-line-format)
        (setq where (car nsevil-mode-line-format)
              which (cdr nsevil-mode-line-format))))
      ...
      (while (and mlpos
        ...
        (setq pred mlpos
              mlpos (cdr mlpos)))
      ;; put nsevil tag at the right position in the mode line
      (cond
       ((not mlpos)) ;; position not found, so do not add the tag
       ((eq where 'before)
        (if pred
            (setcdr pred (cons 'nsevil-mode-line-tag mlpos))
          (setq mode-line-format
                (cons 'nsevil-mode-line-tag mode-line-format))))
       ((eq where 'after)
        (setcdr mlpos (cons 'nsevil-mode-line-tag (cdr mlpos)))))
      (force-mode-line-update))))

;; input methods should be disabled in non-insertion states
(defun nsevil-activate-input-method ()
  "Enable input method in states with :input-method non-nil."
  (let (input-method-activate-hook
        input-method-deactivate-hook)
    (when (and nsevil-local-mode nsevil-state)
      (setq nsevil-input-method current-input-method)
      (unless (nsevil-state-property nsevil-state :input-method)
        (deactivate-input-method)))))
(put 'nsevil-activate-input-method 'permanent-local-hook t)

(defun nsevil-deactivate-input-method ()
  "Disable input method in all states."
  (let (input-method-activate-hook
        input-method-deactivate-hook)
    (when (and nsevil-local-mode nsevil-state)
      (setq nsevil-input-method nil))))
(put 'nsevil-deactivate-input-method 'permanent-local-hook t)

(defmacro nsevil-without-input-method-hooks (&rest body)
  "Execute body with nsevil's activate/deactivate-input-method hooks deactivated.
  ...
  `(unwind-protect
       (progn
         ...
         (remove-hook 'input-method-deactivate-hook
                      'nsevil-deactivate-input-method t)
         ,@body)
     (progn
       (add-hook 'input-method-activate-hook 'nsevil-activate-input-method nil t)
       (add-hook 'input-method-deactivate-hook
                 'nsevil-deactivate-input-method nil t))))

(defadvice toggle-input-method (around nsevil)
  "Refresh `nsevil-input-method'."
  (cond
   ((not nsevil-local-mode)
    ad-do-it)
   ((nsevil-state-property nsevil-state :input-method)
    ad-do-it)
   (t
    (let ((current-input-method nsevil-input-method))
      ad-do-it))))
...
higher precedence. See also `nsevil-make-intercept-map'."
...
(defun nsevil-make-intercept-map (keymap &optional state aux)
  "Give KEYMAP precedence over all Nsevil keymaps in STATE.
...
KEYMAP itself. See also `nsevil-make-overriding-map'."
  (let ((key [intercept-state])
        (keymap (if aux
                    (nsevil-get-auxiliary-keymap keymap state t t)
                  keymap)))
    (define-key keymap key (or state 'all))))

(defmacro nsevil-define-keymap (keymap doc &rest body)
  "Define a keymap KEYMAP listed in `nsevil-mode-map-alist'.
...
\(fn KEYMAP DOC [[KEY VAL]...] BODY...)"
  ...
  (let ((func t)
    ...
    `(progn
       ...
       (when ,intercept
         (nsevil-make-intercept-map ,keymap))
       (when ,overriding
         (nsevil-make-overriding-map ,keymap))
       ,@(if local
             `((make-variable-buffer-local ',keymap)
               (put ',keymap 'permanent-local t)
               (nsevil--add-to-alist 'nsevil-local-keymaps-alist
                                  ',mode ',keymap))
           `((nsevil--add-to-alist 'nsevil-global-keymaps-alist
                                ',mode ',keymap)
             (nsevil--add-to-alist 'nsevil-mode-map-alist
                                ',mode ,keymap)))
       ,(when (or body func)
...
(defun nsevil-esc-mode (&optional arg)
  "Toggle interception of \\e (escape).
Enable with positive ARG and disable with negative ARG.

When enabled, `nsevil-esc-mode' modifies the entry of \\e in
`input-decode-map'. If such an event arrives, it is translated to
a plain 'escape event if no further event occurs within
`nsevil-esc-delay' seconds. Otherwise no translation happens and
the ESC prefix map (i.e. the map originally bound to \\e in
`input-decode-map`) is returned."
  (cond
   ((or (null arg) (eq arg 0))
    (nsevil-esc-mode (if nsevil-esc-mode -1 +1)))
   ((> arg 0)
    (unless nsevil-esc-mode
      (setq nsevil-esc-mode t)
      (add-hook 'after-make-frame-functions #'nsevil-init-esc)
      (mapc #'nsevil-init-esc (frame-list))))
   ((< arg 0)
    (when nsevil-esc-mode
      (remove-hook 'after-make-frame-functions #'nsevil-init-esc)
      (mapc #'nsevil-deinit-esc (frame-list))
      (setq nsevil-esc-mode nil)))))

(defun nsevil-init-esc (frame)
  "Update `input-decode-map' in terminal."
  (with-selected-frame frame
    (let ((term (frame-terminal frame)))
      (when (and
             (or (eq nsevil-intercept-esc 'always)
                 (and nsevil-intercept-esc
                      (eq (terminal-live-p term) t))) ; only patch tty
             (not (terminal-parameter term 'nsevil-esc-map)))
        (let ((nsevil-esc-map (lookup-key input-decode-map [?\e])))
          (set-terminal-parameter term 'nsevil-esc-map nsevil-esc-map)
          (define-key input-decode-map [?\e]
            `(menu-item "" ,nsevil-esc-map :filter ,#'nsevil-esc)))))))

(defun nsevil-deinit-esc (frame)
  "Restore `input-decode-map' in terminal."
  (with-selected-frame frame
    (let ((term (frame-terminal frame)))
      (when (terminal-live-p term)
        (let ((nsevil-esc-map (terminal-parameter term 'nsevil-esc-map)))
          (when nsevil-esc-map
            (define-key input-decode-map [?\e] nsevil-esc-map)
            (set-terminal-parameter term 'nsevil-esc-map nil)))))))

(defun nsevil-esc (map)
...
this function waits for `nsevil-esc-delay' seconds for another
event. If no other event arrives, the event is translated to
'escape, otherwise it is translated to the standard ESC prefix
map stored in `input-decode-map'. If `nsevil-inhibit-esc' is
non-nil or if nsevil is in emacs state, the event is always
...
mapping will always be the ESC prefix map."
  (if (and (not nsevil-inhibit-esc)
           (or nsevil-local-mode (nsevil-ex-p)
               (active-minibuffer-window))
           (not (nsevil-emacs-state-p))
           (let ((keys (this-single-command-keys)))
             (and (> (length keys) 0)
                  (= (aref keys (1- (length keys))) ?\e)))
           (sit-for nsevil-esc-delay))
      (prog1 [escape]
...
(defun nsevil-state-p (sym)
  "Whether SYM is the name of a state."
  (assq sym nsevil-state-properties))

(defun nsevil-state-keymaps (state &rest excluded)
...
infinite recursion, keeping track of processed states.)"
  (let* ((state (or state nsevil-state))
         (enable (nsevil-state-property state :enable))
         (map (cons
               (nsevil-state-property state :mode)
               (nsevil-state-property state :keymap t)))
         (local-map (cons
                     (nsevil-state-property state :local)
                     (nsevil-state-property state :local-keymap t)))
         (minor-mode-maps (nsevil-state-minor-mode-keymaps state))
         (aux-maps (nsevil-state-auxiliary-keymaps state))
         (overriding-maps
          (nsevil-state-overriding-keymaps state))
         (intercept-maps
          (nsevil-state-intercept-keymaps state))
      ...
      (cond
       ...
       ((eq entry state)
        (setq result `(,@result
                       ...
                       (,map)))
        (push state excluded))
       ;; the keymaps for another state: call `nsevil-state-keymaps'
       ;; recursively, but keep track of processed states
       ((nsevil-state-p entry)
        (setq result `(,@result
                       ,(apply #'nsevil-state-keymaps entry excluded))))
       ;; a single keymap
       ((or (keymapp entry)
            (and (keymapp (symbol-value entry))
                 (setq entry (symbol-value entry)))
            (setq entry (nsevil-keymap-for-mode entry)))
        (setq result `(,@result
                       ((,(nsevil-mode-for-keymap entry t) .
                         ,entry)))))))
    ;; postpone the expensive filtering of duplicates to the top level
    (if remove-duplicates
        (apply #'nsevil-concat-keymap-alists result)
      (apply #'append result))))

(defun nsevil-normalize-keymaps (&optional state)
  "Create a buffer-local value for `nsevil-mode-map-alist'.
...
\(or by STATE if specified)."
  (let ((state (or state nsevil-state))
    ...
    (dolist (entry nsevil-mode-map-alist)
      ...
      (if (or (memq mode excluded)
              (nsevil-intercept-keymap-p map)
              (nsevil-overriding-keymap-p map)
              (nsevil-auxiliary-keymap-p map)
              (nsevil-minor-mode-keymap-p map))
          (push mode excluded)
        (when (and (fboundp mode) (symbol-value mode))
          (funcall mode -1))
        (set mode nil)))
    (setq nsevil-mode-map-alist nil)
    ;; activate keymaps of current state
    (when state
      (setq temp (nsevil-state-keymaps state))
      (dolist (entry temp)
        (setq mode (car entry)
              map (cdr entry))
        ...
        ;; (e.g., `nsevil-operator-shortcut-map' is
        ;; reset on toggling)
        (if (or (memq mode excluded)
                (nsevil-intercept-keymap-p map)
                (nsevil-overriding-keymap-p map)
                ...
                (nsevil-minor-mode-keymap-p map))
            (push mode excluded)
          (setcdr entry (or (nsevil-keymap-for-mode mode) map))))
      ;; update `nsevil-mode-map-alist'
      (setq nsevil-mode-map-alist temp))))

(defun nsevil-mode-for-keymap (keymap &optional default)
  "Return the minor mode associated with KEYMAP.
Returns DEFAULT if no mode is found.
See also `nsevil-keymap-for-mode'."
  (let ((map (if (keymapp keymap) keymap (symbol-value keymap)))
        (var (when (symbolp keymap) keymap)))
    ;; Check Nsevil variables first for speed purposes.
    ;; If all else fails, check `minor-mode-map-alist'.
    (or (when var
          (or (car (rassq var nsevil-global-keymaps-alist))
              (car (rassq var nsevil-local-keymaps-alist))))
        (car (rassq map (mapcar #'(lambda (e)
                                    ;; from (MODE-VAR . MAP-VAR)
                                    ;; to (MODE-VAR . MAP)
                                    (cons (car-safe e)
                                          (symbol-value (cdr-safe e))))
                                (append nsevil-global-keymaps-alist
                                        nsevil-local-keymaps-alist))))
        (car (rassq map minor-mode-map-alist))
        default)))

(defun nsevil-keymap-for-mode (mode &optional variable)
  "Return the keymap associated with MODE.
Return the keymap variable if VARIABLE is non-nil.
See also `nsevil-mode-for-keymap'."
  (let* ((var (or (cdr (assq mode nsevil-global-keymaps-alist))
                  (cdr (assq mode nsevil-local-keymaps-alist))))
         (map (or (symbol-value var)
                  (cdr (assq mode minor-mode-map-alist)))))
    (if variable var map)))

(defun nsevil-state-auxiliary-keymaps (state)
  "Return a keymap alist of auxiliary keymaps for STATE."
  (let ((state (or state nsevil-state))
        aux result)
    (dolist (map (current-active-maps) result)
      (when (setq aux (nsevil-get-auxiliary-keymap map state))
        (push (cons (nsevil-mode-for-keymap map t) aux) result)))
    (nreverse result)))

(defun nsevil-state-minor-mode-keymaps (state)
  "Return a keymap alist of minor-mode keymaps for STATE."
  (let* ((state (or state nsevil-state))
         (state-entry (assq state nsevil-minor-mode-keymaps-alist)))
    (when state-entry
      (cdr state-entry))))

(defun nsevil-state-overriding-keymaps (&optional state)
  "Return a keymap alist of overriding keymaps for STATE."
  (let* ((state (or state nsevil-state))
         result)
    (dolist (map (current-active-maps))
      (when (setq map (nsevil-overriding-keymap-p map state))
        (push (cons (nsevil-mode-for-keymap map t) map) result)))
    (nreverse result)))

(defun nsevil-state-intercept-keymaps (&optional state)
  "Return a keymap alist of intercept keymaps for STATE."
  (let* ((state (or state nsevil-state))
         result)
    (dolist (map (current-active-maps))
      (when (setq map (or (nsevil-intercept-keymap-p map state)
                          (nsevil-intercept-keymap-p
                           (nsevil-get-auxiliary-keymap map state) state)))
        (push (cons (nsevil-mode-for-keymap map t) map) result)))
    (setq result (nreverse result))
    result))

(defun nsevil-set-auxiliary-keymap (map state &optional aux)
  "Set the auxiliary keymap for MAP in STATE to AUX.
If AUX is nil, create a new auxiliary keymap."
  (unless (keymapp aux)
    (setq aux (make-sparse-keymap)))
  (unless (nsevil-auxiliary-keymap-p aux)
    (nsevil-set-keymap-prompt
     aux (format "Auxiliary keymap for %s"
                 (or (nsevil-state-property state :name)
                     (format "%s state" state)))))
  (define-key map
    (vconcat (list (intern (format "%s-state" state)))) aux)
  aux)
(put 'nsevil-set-auxiliary-keymap 'lisp-indent-function 'defun)

(defun nsevil-get-auxiliary-keymap (map state &optional create ignore-parent)
    ...
    (let* ((key (vconcat (list (intern (format "%s-state" state)))))
      ...
      (cond
       ((and ignore-parent
             ...
             create)
        (nsevil-set-auxiliary-keymap map state))
       ((nsevil-auxiliary-keymap-p aux)
        aux)
       (create
        (nsevil-set-auxiliary-keymap map state))))))

(defun nsevil-get-minor-mode-keymap (state mode)
  "Get the auxiliary keymap for MODE in STATE, creating one if it
does not already exist."
  (let ((state-entry (assq state nsevil-minor-mode-keymaps-alist)))
    (if (and state-entry
...
(defun nsevil-overriding-keymap-p (map &optional state)
  ...
  (let ((entry (and (keymapp map)
    ...
    (cond
     ((null entry)
      nil)
     ((keymapp entry)
      (nsevil-overriding-keymap-p entry state))
     ((null state)
      map)
     ((eq entry state)
      map)
...
(defun nsevil-intercept-keymap-state (map)
...
A return value of t means all states."
  (let ((state (lookup-key map [intercept-state] map)))
    (cond
     ((keymapp state)
      (nsevil-intercept-keymap-state state))
     ((eq state 'all)
      t)
     (t
      state))))

(defun nsevil-overriding-keymap-state (map)
  "Return the state for the overriding keymap MAP.
A return value of t means all states."
  (let ((state (lookup-key map [override-state] map)))
    (cond
     ((keymapp state)
      (nsevil-overriding-keymap-state state))
     ((eq state 'all)
      t)
     (t
      state))))

(defun nsevil-send-leader ()
...
(defun nsevil-set-leader (state key &optional localleader)
...
instead."
  (let* ((all-states '(normal insert visual replace operator motion emacs))
         (states (cond ((listp state) state)
                       ...
                       (t (list state))))
         (binding (if localleader 'nsevil-send-localleader 'nsevil-send-leader)))
    (dolist (state states)
      (nsevil-global-set-key state key binding))))

(defmacro nsevil-define-key (state keymap key def &rest bindings)
...
like those of `define-key'. For example:
    ...
    (nsevil-define-key 'normal foo-map \"a\" 'bar)
...
global nsevil keymap corresponding to the state(s) is used, meaning
the following lead to identical bindings:

    (nsevil-define-key 'normal 'global \"a\" 'bar)
    (nsevil-global-set-key 'normal \"a\" 'bar)

The symbol `local' may also be used, which corresponds to using
`nsevil-local-set-key'. If a quoted symbol is used that is not
`global' or `local', it is assumed to be the name of a minor
mode, in which case `nsevil-define-minor-mode-key' is used."
  (declare (indent defun))
  (cond ((member keymap '('global 'local))
         `(nsevil-define-key* ,state ,keymap ,key ,def ,@bindings))
        ((and (consp keymap) (eq (car keymap) 'quote))
         `(nsevil-define-minor-mode-key ,state ,keymap ,key ,def ,@bindings))
        (t
         `(nsevil-delay ',(if (symbolp keymap)
                            `(and (boundp ',keymap) (keymapp ,keymap))
                          `(keymapp ,keymap))
              '(condition-case-unless-debug err
                   (nsevil-define-key* ,state ,keymap ,key ,def ,@bindings)
                 (error
                  (message "error in nsevil-define-key: %s"
                           (error-message-string err))))
            'after-load-functions t nil
            (format "nsevil-define-key-in-%s"
                    ',(if (symbolp keymap) keymap 'keymap))))))
(defalias 'nsevil-declare-key 'nsevil-define-key)

(defun nsevil-define-key* (state keymap key def &rest bindings)
  "Create a STATE binding from KEY to DEF for KEYMAP.
...
The symbol local may also be used, which corresponds to using
`nsevil-local-set-key'.

The use is nearly identical to `nsevil-define-key' with the
exception that this is a function and not a macro (and so will
not be expanded when compiled which can have unintended
consequences). `nsevil-define-key*' also does not defer any
bindings like `nsevil-define-key' does using `nsevil-delay'. This
...
its behavior more predictable."
  (let ((maps
             ...
             (mapcar
              (lambda (st)
                (cond ((eq keymap 'global)
                       (nsevil-state-property st :keymap t))
                      ((eq keymap 'local)
                       (nsevil-state-property st :local-keymap t))
                      (t
                       (nsevil-get-auxiliary-keymap keymap st t t))))
    ...
    (dolist (map maps)
      (nsevil-set-keymap-prompt map (keymap-prompt map)))))

(defun nsevil-define-minor-mode-key (state mode key def &rest bindings)
  "Similar to `nsevil-define-key' but the bindings are associated
...
nsevil to use Emacs' built-in mechanisms to enable the bindings
automatically when MODE is active without relying on calling
`nsevil-normalize-keymaps'. Another less significant difference is
that the bindings can be created immediately, because this
function only uses the symbol MODE and does not rely on its
value.

See `nsevil-define-key' for the usage of STATE, KEY, DEF and
BINDINGS."
  (declare (indent defun))
  (let ((maps (mapcar
               (lambda (st)
                 (nsevil-get-minor-mode-keymap st mode))
               (if (listp state) state (list state)))))
    (while key
      (dolist (map maps)
        (define-key map key def))
...
(defmacro nsevil-add-hjkl-bindings (keymap &optional state &rest bindings)
  "Add \"h\", \"j\", \"k\", \"l\" bindings to KEYMAP in STATE.
Add additional BINDINGS if specified."
  (declare (indent defun))
  `(nsevil-define-key ,state ,keymap
     "h" (lookup-key nsevil-motion-state-map "h")
     "j" (lookup-key nsevil-motion-state-map "j")
     "k" (lookup-key nsevil-motion-state-map "k")
     "l" (lookup-key nsevil-motion-state-map "l")
     ":" (lookup-key nsevil-motion-state-map ":")
     ,@bindings))

;; may be useful for programmatic purposes
(defun nsevil-global-set-key (state key def)
  "Bind KEY to DEF in STATE."
  (define-key (nsevil-state-property state :keymap t) key def))

(defun nsevil-local-set-key (state key def)
  "Bind KEY to DEF in STATE in the current buffer."
  (define-key (nsevil-state-property state :local-keymap t) key def))

;; Advise these functions as they may activate an overriding keymap or
;; a keymap with state bindings; if so, refresh `nsevil-mode-map-alist'.
(defadvice use-global-map (after nsevil activate)
  "Refresh Nsevil keymaps."
  (nsevil-normalize-keymaps))

(defadvice use-local-map (after nsevil activate)
  "Refresh Nsevil keymaps."
  (nsevil-normalize-keymaps))

(defmacro nsevil-define-state (state doc &rest body)
  "Define an Nsevil state STATE.
...
- `:suppress-keymap' - if non-nil, effectively disables bindings to
  `self-insert-command' by making `nsevil-suppress-map' the parent of
  the global state keymap.

The global keymap of this state will be `nsevil-test-state-map',
the local keymap will be `nsevil-test-state-local-map', and so on.
...
\(fn STATE DOC [[KEY VAL]...] BODY...)"
  ...
  (let* ((name (and (string-match "^\\(.+\\)\\(\\(?:.\\|\n\\)*\\)" doc)
         ...
         (doc (if (or (null doc) (string= doc "")) ""
                (format "\n%s" doc)))
         (toggle (intern (format "nsevil-%s-state" state)))
    ...
    ;; macro expansion
    `(progn
       ;; Save the state's properties in `nsevil-state-properties' for
...
cursor, or a list of the above." name))
       ...
       ,@(when suppress-keymap
           `((set-keymap-parent ,keymap nsevil-suppress-map)))
...
\(That is, whether `nsevil-state' is `%s'.)" name state)
         (and nsevil-local-mode
              (eq (or state nsevil-state) ',state)))
...
If ARG is nil, don't display a message in the echo area.%s" name doc)
         ...
         (cond
          ((and (numberp arg) (< arg 1))
           (setq nsevil-previous-state nsevil-state
                 nsevil-state nil)
           (let ((nsevil-state ',state))
             (run-hooks ',exit-hook)
             (setq nsevil-state nil)
             (nsevil-normalize-keymaps)
             ,@body))
          (t
           (unless nsevil-local-mode
             (nsevil-local-mode 1))
           (let ((nsevil-next-state ',state)
                 input-method-activate-hook
                 input-method-deactivate-hook)
             (nsevil-change-state nil)
             (setq nsevil-state ',state)
             (nsevil--add-to-alist 'nsevil-previous-state-alist
                                ',state nsevil-previous-state)
             (let ((nsevil-state ',state))
               (nsevil-normalize-keymaps)
               (if ',input-method
                   (activate-input-method nsevil-input-method)
               ...
               (unless nsevil-no-display
                 (nsevil-refresh-cursor ',state)
                 (nsevil-refresh-mode-line ',state)
                 (when (called-interactively-p 'any)
                   (redisplay)))
               ,@body
               (run-hooks ',entry-hook)
               (when (and nsevil-echo-state
                          arg (not nsevil-no-display) ,message)
                 (if (functionp ,message)
                     (funcall ,message)
                   (nsevil-echo "%s" ,message))))))))

       (nsevil-set-command-property ',toggle :keep-visual t)
       (nsevil-set-command-property ',toggle :suppress-operator t)

       (nsevil-define-keymap ,keymap nil
         :mode ,mode
         :func nil)

       (nsevil-define-keymap ,local-keymap nil
         :mode ,local
         :local t
         :func nil)
